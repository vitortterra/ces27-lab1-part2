...
func newMaster(address string) (master *Master) {
	...
	master.failedOperation = make(chan bool, 1)
...
func (master *Master) handleFailingWorkers() {
	...
	for failingWorker := range master.failedWorkerChan {
		master.workersMutex.Lock()
		log.Printf("Removing worker '%v' from master list", failingWorker.id)
		delete(master.workers, failingWorker.id)
